...
__kernel void conv2d1x1(
	...
	__global const float8* restrict filter_weight,
	...
	for(int f_i = 0; f_i < filter_size; f_i++)
	{
		...
		for(int ij = 0; ij < (input_size * input_size); ij++)
		{
			...
			for(int k = 0; k < input_channels; k++)
			{
				...
				//float8 feature;
				shift_reg[II_CYCLES - 1] = shift_reg[0] + input_im[((k << 3) + 0) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s0;
				shift_reg[II_CYCLES - 2] = shift_reg[1] + input_im[((k << 3) + 1) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s1;
				shift_reg[II_CYCLES - 3] = shift_reg[2] + input_im[((k << 3) + 2) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s2;
				shift_reg[II_CYCLES - 4] = shift_reg[3] + input_im[((k << 3) + 3) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s3;
                shift_reg[II_CYCLES - 5] = shift_reg[4] + input_im[((k << 3) + 4) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s4;
				shift_reg[II_CYCLES - 6] = shift_reg[5] + input_im[((k << 3) + 5) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s5;
				shift_reg[II_CYCLES - 7] = shift_reg[6] + input_im[((k << 3) + 6) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s6;
				shift_reg[II_CYCLES - 8] = shift_reg[7] + input_im[((k << 3) + 7) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s7;
				
				#pragma unroll
				for(int j = 0; j < (II_CYCLES - 8); j++)
				{
					shift_reg [j] = shift_reg[j + (1 << 3)];
				}
