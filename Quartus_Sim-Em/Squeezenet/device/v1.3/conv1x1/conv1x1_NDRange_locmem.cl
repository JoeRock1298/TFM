	...
	__global float *restrict output_im)
{
	...
	//for(int i = 0; i < input_size; i++)
	{
		for(int j = 0; j < input_size; j++)
		{
			...
			{
				im_in[k].s0 = input_im[((k << 2) + 0) * input_size * input_size + loc];
                im_in[k].s1 = input_im[((k << 2) + 1) * input_size * input_size + loc];
                im_in[k].s2 = input_im[((k << 2) + 2) * input_size * input_size + loc];
                im_in[k].s3 = input_im[((k << 2) + 3) * input_size * input_size + loc];
			}
			...
			for(int k = 0; k < input_channels; k++)
			{
				...
				//float8 feature;
				tmp += im_in[k].s0 * filter_weight[k].s0
				     + im_in[k].s1 * filter_weight[k].s1
					 + im_in[k].s2 * filter_weight[k].s2
					 + im_in[k].s3 * filter_weight[k].s3;
			}
			//add relu after conv
