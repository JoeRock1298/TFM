	...
	for(int f_i = 0; f_i < filter_size; f_i++)
	{
		...
		for(int ij = 0; ij < (input_size * input_size); ij++)
		{
			...
			for(int k = 0; k < input_channels; k++)
			{
				...
				//float8 feature;
				shift_reg[II_CYCLES - 1] = shift_reg[0] + input_im[((k << 2) + 0) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s0;
				shift_reg[II_CYCLES - 2] = shift_reg[1] + input_im[((k << 2) + 1) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s1;
				shift_reg[II_CYCLES - 3] = shift_reg[2] + input_im[((k << 2) + 2) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s2;
				shift_reg[II_CYCLES - 4] = shift_reg[3] + input_im[((k << 2) + 3) * input_size * input_size + ij] * filter_weight[k + f_i * input_channels].s3;
				
				#pragma unroll
				for(int j = 0; j < (II_CYCLES - 4); j++)
				{
					shift_reg [j] = shift_reg[j + (1 << 2)];
				}
