...
module conv2d3x3_function_cra_slave (
    ...
    output wire [447:0] kernel_arguments,
    ...
    wire [447:0] kernel_arg_bit_join_q;
    ...
    // not_finished(LOGICAL,130)
    assign not_finished_q = ~ (finish);

    // compute_running_not_done(LOGICAL,53)
    ...
    // printf_bit(BITSELECT,163)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,164)
    ...
    // readdata_valid_stage1_reg(REG,171)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,67)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,62)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,59)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,142)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,172)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,60)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,83)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,77)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,85)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,79)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,87)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,81)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,92)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,94)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,96)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // kernel_arg_bit_join(BITJOIN,117)
    assign kernel_arg_bit_join_q = {dupName_2_arg_bit_join_x_q, dupName_1_arg_bit_join_x_q, dupName_0_arg_bit_join_x_q, arg_bit_join_q, arguments_5_q, arguments_4_q, arguments_3_q, arguments_2_q, arguments_1_q, arguments_0_q};

    // kernel_arguments(GPOUT,151)
    assign kernel_arguments = kernel_arg_bit_join_q;

    // dupName_9_address_ref_x(CONSTANT,254)
    assign dupName_9_address_ref_x_q = $unsigned(5'b01010);

    // dupName_9_NO_NAME_x(LOGICAL,253)
    assign dupName_9_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_9_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_9_can_write_x(LOGICAL,255)
    assign dupName_9_can_write_x_q = dupName_9_NO_NAME_x_q & avs_cra_write;

    // dupName_8_mask0_x(LOGICAL,250)
    assign dupName_8_mask0_x_q = bit_enable_bottom_bar_b & local_size_reg_0_buffered_q;

    // dupName_8_new_data_x(LOGICAL,252)
    assign dupName_8_new_data_x_q = dupName_8_mask0_x_q | mask1_q;

    // local_size_reg_0_buffered(REG,120)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,204)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
