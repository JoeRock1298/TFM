...
// SystemVerilog created from mmul_function_cra_slave
...
module mmul_function_cra_slave (
    ...
    output wire [287:0] kernel_arguments,
    ...
    wire [287:0] kernel_arg_bit_join_q;
    ...
    // printf_bit(BITSELECT,153)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,154)
    ...
    // readdata_valid_stage1_reg(REG,161)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,57)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // start_bit_computation(LOGICAL,175)
    assign start_bit_computation_q = not_start_q & start_bit_b;

    // status_self_update(BITJOIN,188)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,52)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,49)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,132)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // readdata_valid_stage2_reg(REG,162)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,50)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,73)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,67)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,75)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,69)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,77)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,71)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,82)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,84)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,86)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,203)
    assign dupName_1_arg_bit_join_x_q = {arguments_6_q, arguments_5_q};

    // dupName_21_mask0_x(LOGICAL,307)
    assign dupName_21_mask0_x_q = bit_enable_bottom_bar_b & arguments_4_buffered_q;

    // dupName_21_new_data_x(LOGICAL,309)
    ...
    // dupName_0_arg_bit_join_x(BITJOIN,196)
    assign dupName_0_arg_bit_join_x_q = {arguments_4_q, arguments_3_q};

    // dupName_19_mask0_x(LOGICAL,301)
    assign dupName_19_mask0_x_q = bit_enable_bottom_bar_b & arguments_2_buffered_q;

    // dupName_19_new_data_x(LOGICAL,303)
    ...
    // kernel_arg_bit_join(BITJOIN,107)
    assign kernel_arg_bit_join_q = {arguments_8_q, arguments_7_q, dupName_1_arg_bit_join_x_q, dupName_0_arg_bit_join_x_q, arg_bit_join_q, arguments_0_q};

    // kernel_arguments(GPOUT,141)
    assign kernel_arguments = kernel_arg_bit_join_q;

    // dupName_9_address_ref_x(CONSTANT,242)
    assign dupName_9_address_ref_x_q = $unsigned(5'b01010);

    // dupName_9_NO_NAME_x(LOGICAL,241)
    assign dupName_9_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_9_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_9_can_write_x(LOGICAL,243)
    assign dupName_9_can_write_x_q = dupName_9_NO_NAME_x_q & avs_cra_write;

    // dupName_8_mask0_x(LOGICAL,238)
    assign dupName_8_mask0_x_q = bit_enable_bottom_bar_b & local_size_reg_0_buffered_q;

    // dupName_8_new_data_x(LOGICAL,240)
    assign dupName_8_new_data_x_q = dupName_8_mask0_x_q | mask1_q;

    // local_size_reg_0_buffered(REG,110)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,194)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
