...
// SystemVerilog created from double_add_2_function_cra_slave
...
module double_add_2_function_cra_slave (
    ...
    output wire [159:0] kernel_arguments,
    ...
    wire [159:0] kernel_arg_bit_join_q;
    ...
    // readdata_valid_stage1_reg(REG,153)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,49)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // const_finish_counter_padding(CONSTANT,39)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,44)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,41)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // readdata_valid_stage2_reg(REG,154)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,42)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,65)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,59)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_0_bit_join(BITJOIN,56)
    ...
    // global_offset_reg_upper_1(REG,67)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,61)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_1_bit_join(BITJOIN,57)
    ...
    // global_offset_reg_upper_2(REG,69)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,63)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_2_bit_join(BITJOIN,58)
    ...
    // global_size_reg_upper_0(REG,80)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,74)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_0_bit_join(BITJOIN,71)
    ...
    // global_size_reg_lower_1(REG,76)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_1_bit_join(BITJOIN,72)
    ...
    // global_size_reg_lower_2(REG,78)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_2_bit_join(BITJOIN,73)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,186)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
